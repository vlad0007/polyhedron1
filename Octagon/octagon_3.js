...
{	
    ...
    controller = new function() 
	{
	  ...
	  this.hMiddleFacet = hMiddleFacet;
	  this.angle_D = angle_D / DEGREE;	
    ...
    f3.add(controller, 'angle_D', 10.0, 89.0).onChange( function() 
	{
		var temp = angle_D;
		orbitControl.enabled = false;
		angle_D = (controller.angle_D)*DEGREE;
		recalc();   
	    if (isCorrect() == -1) 
	    {
		   angle_D = temp;
		   recalc();
		   controller.angle_D = temp / DEGREE;
	    }			
		gui.updateDisplay();
    });
...
function pars_value()
{
	...
	h = h + dh;
	text = "Pavilion angle D ";
	ctx.fillStyle = text_color;
	ctx.fillText(text, w1, h);	
	text = roundNumber(angle_D/DEGREE, 3) + "°";
	ctx.fillStyle = value_color;
	ctx.fillText(text, w2, h);
