...
function pars_value()
{
	...
	ctx.fillStyle = text_color;
	ctx.fillText(text, w1, h);		
	text = roundNumber(crown_middle_diameter, 2);
	ctx.fillStyle = value_color;
	ctx.fillText(text, w2, h);	
	...
	h = h + dh;
	var text = "Crown facet down height ";
	ctx.fillStyle = text_color;
	ctx.fillText(text, w1, h);		
	text = roundNumber(hCrownDown, 2);
	ctx.fillStyle = value_color;
	ctx.fillText(text, w2, h);
	...
	h = h + dh;
	text = "Angle slope facet D0 - D7(ang_pav) ";
	ctx.fillStyle = text_color;
	ctx.fillText(text, w1, h);	
	text = roundNumber(ang_pav / DEGREE, 3) + "°";
	ctx.fillStyle = value_color;
	ctx.fillText(text, w2, h);
	
	h = h + dh;
	var text = "Pavilion down level ";
	ctx.fillStyle = text_color;
	ctx.fillText(text, w1, h);		
	text = roundNumber(DownCleanLevel, 2);
	ctx.fillStyle = value_color;
	ctx.fillText(text, w2, h);	
	
	h = h + dh;
	var text = "Pavilion facet 1 ";
	ctx.fillStyle = text_color;
	ctx.fillText(text, w1, h);		
	text = roundNumber(hPavFacet1, 2);
	ctx.fillStyle = value_color;
	ctx.fillText(text, w2, h);	
	
	h = h + dh;
	var text = "Pavilion facet 2 ";
	ctx.fillStyle = text_color;
	ctx.fillText(text, w1, h);		
	text = roundNumber(hPavFacet2, 2);
	ctx.fillStyle = value_color;
	ctx.fillText(text, w2, h);	
/*	
    if (error == true)
...
function print_text_index(index) // 100, 130, 160
{
	...
	if (index == 105)
	{
		ctx.fillText("    Pavilion facet  G15", left_2, h1);
	}
	if (index == 106)
	{
	...
	if (index == 118)
	{
		ctx.fillText("    Pavilion facet  G12", left_2, h1);
	}	
	if (index == 119)
	{
		ctx.fillText("    Pavilion facet  G13", left_2, h1);
	}	
	if (index == 120)
	{
		ctx.fillText("    Pavilion facet  G14", left_2, h1);
	}	
	/////////////////////////////////////////////////
	if (index == 121)
	{
	...
	if (index == 142)
	{
		ctx.fillText("    Pavilion facet  F5", left_2, h1);
	}	
	if (index == 143)
	{
		ctx.fillText("    Pavilion facet  F6", left_2, h1);
	}
	if (index == 144)
	{
		ctx.fillText("    Pavilion facet  F7", left_2, h1);
	}	
...
function create_num_vertices(enumeration)
{
	...
	for (i = 0; i < n; i++) 
	{
		...
		if  ( (i > 32)&&(i < 36)   || (i > 36)&&(i < 40)   || (i > 40)&&(i < 44)   || (i > 44)&&(i < 48)   || (i > 48)&&(i < 52)   ||
			  (i > 52)&&(i < 56)   || (i > 56)&&(i < 60)   || (i > 60)&&(i < 64)   || (i > 64)&&(i < 68)   || (i > 68)&&(i < 72)   ||
			  (i > 72)&&(i < 76)   || (i > 76)&&(i < 80)   || (i > 80)&&(i < 84)   ||
			  (i > 84)&&(i < 88)   || (i > 88)&&(i < 92)   || (i > 92)&&(i < 96)   ||
		...
		if (enumeration == 2)
		{
			...
			else 
			{
				...
				generateGeometry( meshText, (number - 160).toString() );	
			}			
		}
